...
*/
function viewStudentStatus(id , toggle){
	if(toggle == 0){
...
function loadStatsPage(){
	...
	for(row_counter=2; row_counter < table.rows.length; row_counter++){
		...
		if(homeScore > oppScore){
			...
			table.rows[row_counter].cells[4].innerHTML = team;
			losses++;
		}
	}
...
/*
I tried to get this function but nothing seemed to work with either an anchor tag or option but I think that the
logic is there it just wont append to the end of the dropdown 
*/
function loadPlayersPage(){
	...
	for(counter = 0; counter < players.length; counter++){
		...
		x.text = counter;
		x.value = counter;
		x.onclick = switchPlayers(counter);
		select.add(x);
	}
...
function switchPlayer(playerNum){
	...
	document.getElementById(avg_p_yards).innerHTML = (p.pass_yards / p.games_played);
	document.getElementById(avg_r_yards).innerHTML = (p.rushing_yards / p.games_played);
	document.getElementById(avg_rec_yards).innerHTML = (p.receiving_yards / p.games_played);
